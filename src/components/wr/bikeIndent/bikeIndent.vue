  ...
  <div id="content">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="行程状态">
        <template slot-scope="scope">
          <div v-if="scope.row.distance_status==1||scope.row.distance_status==2||scope.row.distance_status==3">
            <span style="color: orange;">进行中</span>
          </div>
          <div v-if="scope.row.distance_status==4">
            <span style="color:orange;">待支付</span>
          </div>
          <div v-if="scope.row.distance_status==5||scope.row.distance_status==6">
  ...
  export default {
    ...
    methods: {
      getTabData(page) {
        ...
        const url = "wurengongxiangdanche/Distance/RetrieveListByParms";
      ...
      openDistance(row) {
        console.log(row.distance_id);
        this.$router.push({name: 'bikeIndentItem', query: {distance_id: row.distance_id}});
      }
    },
    mounted() {
      this.getTabData();
